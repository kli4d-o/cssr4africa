...
bool execute_gesture(cssr_system::performGesture::Request& service_request, cssr_system::performGesture::Response& service_response)
{
    ...
    if (verbose_mode) {
        ROS_INFO(
            "%s: request to /gestureExecution/perform_gesture service: \
    ...
    string arm = service_request.arm;
    string deicticShape = service_request.deictic_shape;
    ...
    if ((gesture_type == DEICTIC_GESTURES) || (gesture_type == DIECTIC_GESTURES)) {
        ...
        execution_status = deictic_gesture(arm, deicticShape == "PALM_UPWARDS", point_location_x, point_location_y, point_location_z, gesture_duration, topics_file_name, interpolation_mode, gesture_velocity_publisher, verbose_mode);
    }
...
bool execute_gesture_action(const actionlib::SimpleActionServer<cssr_system::performGestureAction>::GoalConstPtr& goal, actionlib::SimpleActionServer<cssr_system::performGestureAction>* as)
{
    int execution_status = 0;  // Stores the status of the gesture execution
    bool debug = true;         // Debug mode flag

    cssr_system::performGestureFeedback feedback;
    cssr_system::performGestureResult result;

    result.gesture_success = execution_status;  // Set the default response to the service
    ...
    string arm = goal->arm;
    string deicticShape = goal->deictic_shape;
    ...
    /* Symbolic Gestures Execution */
        ...
        ROS_WARN("%s: symbolic gesture execution not implemented yet.", node_name.c_str());
        result.gesture_success = 0;  // Set the response to the service
        return true;
    }
...
// Construct the full path of the configuration file
    ...
    if (debug_mode)
        ROS_INFO("%s: robot pose input data file is %s", node_name.c_str(), data_path_and_file.c_str());
...
// Construct the full path of the configuration file
    ...
    if (debug_mode)
        ROS_INFO("%s: configuration file is %s", node_name.c_str(), config_path_and_file.c_str());
...
// Construct the full path of the configuration file
    ...
    if (debug_mode)
        ROS_INFO("%s: gesture descriptors file is %s", node_name.c_str(), data_path_and_file.c_str());
...
// Construct the full path of the configuration file
    ...
    if (debug_mode)
        ROS_INFO("%s: gesture descriptors configuration file is %s", node_name.c_str(), data_path_and_file.c_str());
...
// Construct the full path of the topic file
    ...
    if (debug)
        ROS_INFO("%s: topic file is %s", node_name.c_str(), topic_path_and_file.c_str());
...
int deictic_gesture(string arm, bool palm_upwards, float point_x, float point_y, float point_z, int gesture_duration, string topics_file, int interpolation, ros::Publisher velocity_publisher, bool debug)
{
    ...
    ROS_INFO_STREAM("The selected arm is " << arm << " and the palm is " << palm_upwards);
    ...
    ROS_INFO_STREAM("pointing_y: " << pointing_y);
    if (arm == "EITHER") {
        if (pointing_x >= 0.0) {
        ...
        // Only rotate if target is on the wrong side or behind
        if (pointing_x < 0.0 || pointing_y <= 0.0) {
            pose_achievable = false;
            double bearing = atan2(pointing_y, pointing_x);
            double arm_offset = 15.0 * M_PI / 180.0;
            double R = bearing - arm_offset;
            rotation_angle = R * 180.0 / M_PI;

            // Transform pointing coordinates into the post-rotation frame
            pointing_x = pointing_x * cos(R) + pointing_y * sin(R);
            pointing_y = -pointing_x * sin(R) + pointing_y * cos(R);
        }
    } else if (arm == "RIGHT") {
        pointing_arm = RIGHT_ARM;
        y_s = -y_s;
        ...
        ROS_ERROR_STREAM("Invalid Arm Configuration Specified");
        return 0;
    }
    ...
    if (pointing_arm == RIGHT_ARM) {  // Right arm pointing gesture
        ...
        wrist_yaw = palm_upwards ? 1 : -1;  // Value for wristYaw for pointing gesture
