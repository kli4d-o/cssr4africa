...
int main(int argc, char** argv)
{
    ...
    if (read_gesture_execution_test_configuration(&implementation_platform, &interface, &verbose_mode) != 0) {
        ROS_ERROR("%s: error reading the gesture execution test configuration file.", node_name.c_str());
        shut_down_handler(0);
        return 0;
    }

    ROS_INFO_STREAM("gestureExecutionTest: selected interface: " << interface);
